...
contract CrowdFund {
    ...
    function payCampaign(
        uint _CampaignId
    ) public payable activeTime(CampaignId) {
    ...
    function getContributorsBalance(
        address _addr,
        uint _CampaignId
    ...
    function getMap(
        uint _id
    ) external view returns (Campaign memory _mapcampaign) {
        _mapcampaign = mapCampaign[_id];
    }
